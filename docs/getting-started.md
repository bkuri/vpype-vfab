...
2. [Understanding ploTTY](#understanding-plotty)
...
vpype circle --radius 1cm plotty-add --name test
```

## Understanding ploTTY

### What is ploTTY?

[ploTTY](https://github.com/bkuri/plotty) is a professional plotter management system that provides:
...
### ploTTY vs Standalone Mode

**ploTTY Integration** (Recommended):
...
- Works without ploTTY installation
...
# 3. Add to ploTTY
vpype plotty-add --name simple_circle --preset default --queue

# 4. Check status
vpype plotty-status --name simple_circle
```
...
vpype plotty-add --name my_design --preset hq --paper A3 --queue
...
for file in designs/*.svg; do
    ...
    vpype read "$file" plotty-add --name "$name" --preset fast --queue
done
...
vpype rand --seed 123 plotty-add --name variant_123 --preset fast --queue

# Test plot
vpype plotty-status --name variant_123

# If good, create high-quality version
vpype rand --seed 123 plotty-add --name final_123 --preset hq --queue
```
...
Add current document to ploTTY job system.
...
List ploTTY jobs with filtering.
...
2. **Verify ploTTY**: `ploTTY --version` (if using ploTTY)
3. **Test Workspace**: `vpype plotty-list`
4. **Enable Debug**: `vpype --debug plotty-add --name test`
